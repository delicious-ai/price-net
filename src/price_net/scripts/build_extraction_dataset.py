...
def main(dataset_path: Path):
    ...
    for attribution_id, vals in tqdm(
        price_boxes.groupby(ATTRIBUTION_SET_ID_COL), total=len(num_images)
    ):
        scene_img = Image.open(
            dataset_path / SCENE_IMG_DIR / f"{attribution_id}.{IMG_EXT}"
        )
